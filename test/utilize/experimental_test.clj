(ns utilize.experimental-test
  (:use clojure.test utilize.map utilize.experimental))
      ...
      (with-local-vars [wrappers ()]
        ...
        (let [call-log (atom nil)]
          (is (= 2 (with-bindings {wrappers (list (fn [f]
                                                    (fn [x]
                                                      (let [ret (f x)]
                                                        (reset! call-log [(-> wrappers deref meta :utilize.experimental/call-data :fn-name) x ret])
                                                        ret))))}
                     (my-inc 1))))
